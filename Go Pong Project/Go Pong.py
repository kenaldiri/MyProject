...
BALL_RADIUS = 5
# Memilih Font dan SizenyaS
SCORE_FONT = pygame.font.SysFont('arial', 30)

WINNING_SCORE = 8
# Untuk menmbuat paddle
class Paddle:
    COLOR = "pink"
    VEL = 4
    def __init__(self, x, y, width, height):
        self.x = self.origanal_x = x
        self.y = self.origanal_y = y
        self.width = width
        self.height = height
    ...
    def reset(self):
        self.x = self.origanal_x
        self.y = self.origanal_y
class Ball:
    global COLOR
    MAX_VEL = 9
    COLOR = "pink"
...
def draw(window, paddles, ball, left_score, right_score):
    ...
    # Menggambar dan Mengatur posisi score
    ...
    pygame.draw.rect(window, COLOR, (WIDTH // 2 - 5, 0, 5, HEIGHT)) # MENGATUR GARIS TENGAH
...
# FUNGSI UNTUK MENGATUR KESELURUHAN PROGRAM
def utama():
    run = True  
    ...
    while run:
        ...
        if left_score >= WINNING_SCORE:
            WON = True
            win_txt = "PEMAIN KIRI MENANG !"
        elif right_score >= WINNING_SCORE:
            WON = True
            win_txt = "PEMAIN KANAN MENANG !"

        if WON:
            txt = SCORE_FONT.render(win_txt, 1, "white")
